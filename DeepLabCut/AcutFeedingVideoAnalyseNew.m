function [TrialArray] = AcutFeedingVideoAnalyseNew(DLCFileFolder,FileName,DisplayPlot,Threshold) 
...
Temp=InfoArray(:,2:3);Temp(InfoArray(:,4)<0.85,1:2)=nan;Struct.Head=Temp;clear Temp 
Temp=InfoArray(:,5:6);Temp(InfoArray(:,7)<0.85,1:2)=nan;Struct.Body=Temp;clear Temp
Temp=InfoArray(:,8:9);Temp(InfoArray(:,10)<0.85,1:2)=nan;Struct.TailBase=Temp;clear Temp
Temp=InfoArray(:,11:12);Temp(InfoArray(:,13)<0.85,1:2)=nan;Struct.Jelly=Temp;clear Temp
Temp=InfoArray(:,14:15);Temp(InfoArray(:,16)<0.85,1:2)=nan;Struct.Empty=Temp;clear Temp
% collect data by frames
NewArray=nan(length(InfoArray),10);
NewArray(:,1:2)=Struct.Head;
NewArray(:,3:4)=Struct.Body;
NewArray(:,5:6)=Struct.TailBase;
NewArray(:,7:8)=Struct.Jelly;
NewArray(:,9:10)=Struct.Empty;
% define the arena
MaxX=max([NewArray(:,[1 3 5 7 9 ])]);MaxX=max(MaxX);
MinX=min([NewArray(:,[1 3 5 7 9])]);MinX=min(MinX);
MaxY=max(NewArray(:,[2 4 6 8 10]));MaxY=max(MaxY);
MinY=min(NewArray(:,[2 4 6 8 10 ]));MinY=min(MinY);
...
NewArrayX=NewArray(:,[1:2:9]); % get x values of array
NewArrayY=NewArray(:,[2:2:10]); % get Y values of array
...
NewArray(:,[1:2:9])=NewArrayX; % get x values of array
NewArray(:,[2:2:10])=NewArrayY; % get Y values of array
...
legend(char(fields(1)),char(fields(2)),char(fields(3)),char(fields(4)),char(fields(5)))
    if DisplayPlot
end % if
end %for plot
...
for GetAngles=1:1
XYHead = [NewArray(:,1) NewArray(:,2)]; % head
XYBody = [NewArray(:,3) NewArray(:,4)]; % body
XYTailBase = [NewArray(:,5) NewArray(:,6)]; % tail base
...
FoodPlate=[nanmedian(NewArrayX(:,4)),nanmedian(NewArrayY(:,4))];
% FoodPlate = ginput(1);
% FoodPlate(2)=FoodPlate(2)*-1;
% %
EmptyPlate=[nanmedian(NewArrayX(:,5)),nanmedian(NewArrayY(:,5))];
...
Close2FoodFrames=find([DistancesArray(1).Distance2Food(:,1)]<Threshold); % small distance indicates proximity to plate
Close2FoodFrames=setdiff(Close2FoodFrames,[RearingFrames]); 
Close2EmptyFrames=find([DistancesArray(1).Distance2Empty(:,1)]<Threshold); % small distance indicates proximity to plate
Close2EmptyFrames=setdiff(Close2EmptyFrames,[RearingFrames]); 
LeftTurnFrames=setdiff(GroupAngle(9).Frame,[RearingFrames;Close2FoodFrames]);%90
RightTurnFrames=setdiff(GroupAngle(4).Frame,[RearingFrames;Close2FoodFrames]);%-90
